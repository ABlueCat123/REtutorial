...
譬如，对于常规表达式 $(2+5^3) \times 4 + 3$，写成后缀表达式如下：

`5 3 ^ 2 + 4 * 3 +` （其中^代表乘方运算）

从左到右，遇到**数字**就压入栈内，遇到**运算符**就弹出栈顶元素计算后放回。

对于常规表达式来说，我们是先计算$5^3$，然后与2相加，接着乘以4后再加3。

而后缀表达式的计算过程如下：
1. 先在栈内压入 5 3 两个值；
2. 通过 ^ 运算，弹出栈顶两个值（5 3），计算$5^3$，得到125后压入栈内；
3. 将2压入栈内；
4. 通过 + 运算，弹出栈顶两个值（125 2），计算$125+2$，得到127后压入栈内；
5. 将4压入栈内；
6. 通过 * 运算，弹出栈顶两个值（127 4），计算$127 \times 4$，得到508后压入栈内；
7. 将3压入栈内；
8. 通过 + 运算，弹出栈顶两个值（508 3），计算$508+3$，得到511后压入栈内；
9. 计算完毕，答案便为栈中唯一元素：511。

在这里，我们可以看出，指令的先后顺序，直接决定了运算结果。事实上，Java的操作数栈的执行原理和这是一样的。上文中**压入**操作就是马上就要介绍的`load`族指令。Java的运算过程，实际上就是**操作数栈和局部变量表、常量池的交互过程**。局部变量表将变量给到操作数栈内，操作数栈运算后又还给局部变量表。一些常量则是从常量池内单方面取，操作数栈不返还回去。

上述过程十分重要，希望大家看懂了。

### 二、Code 属性简介
`Code`属性是一个只附着在方法表上面的属性，其描述了该方法所使用的全部字节码指令和全部辅助信息。`Code`属性除了附着在一般方法上之外，还会附着在初始化方法(`<clinit>()`)和构造方法(`<init>()`)上面。但它不会附着在本地方法(native method)和抽象方法(abstract method)上面，因为它们事实上没有方法体。因此我们可以理解为：有方法体的方法一定有`Code`属性。
...
我们可以发现，事实上在实现部分，Java 的字节码指令均是由一个个字节所组成的。
...
如果读者学过汇编语言，那一定学过面向寄存器的指令集。然而，对于Java语言来说，它面向的是操作数栈。即一切指令都建立在对操作数栈的栈帧的基础上。这种结构实际上使得绝大部分指令都不需要额外指定操作数 —— 只需要在操作数栈里面取放即可。例如`dmul`指令，其语义是从操作数栈中取2个`double`类型的数，相乘后压入栈中。
...
#### 加载指令
加载指令可以将局部变量表中相应位置的变量加载到操作数栈中，表现形式为**压入到其栈顶**(push)。包括`Tload`、`Tload_<n>`两种。其中`T`取`i l f d a`中的一种，分别代表取int、取long、取float、取double、取一个对象。（后文不再说明这些单字母的详细含义）。对于`Tload`指令，后面紧跟一个字节，代表要从局部变量表中取的变量的位置。而`Tload_<n>`是以下几种指令的总称：`aload_0 aload_1 aload_2 aload_3`。这些指令显然就是从局部变量表的位置`0~3`的地方取变量，也就是不需要再跟一个位置字节了。（`_<n>`中n的取值就是0,1,2,3，后文不再说明`_<n>`的详细含义）
...
?> **提示** 请务必注意`Taload`和`aload`的区别，这里极易混淆。前者是取数组中类型为T的一个元素，后者是取一个对象。
...
数组的存储指令为**Tastore**，`T`取`a b c d f i l s`中的一种。该指令将从操作数栈中先取出一个**T型变量 x**，然后再取出一个**数组对象 a**（通常以`aload`指令取出），之后再取出一个**整型变量 i**，将 **a[i]** 的值设为 **x** 。

?> **提示** 请务必注意`Tastore`和`astore`的区别，这里极易混淆。前者是存储数组中类型为T的一个元素，后者是存储一个对象。
...
`bipush`、`sipush`分别是压入`byte`和`short`，不同在于，`bipush`后面跟一个字节，表示要压入的立即数；`sipush`后面跟两个字节，表示要压入的数为`(byte1 << 8) | byte2`，也就是两个字节拼接起来的数。

`ldc`、`ldc_w`、`ldc2_w`功能一样，都是从常量池将目标对象压入操作数栈。`ldc`后面只跟一个字节，这限制取值为 0~255，而`ldc_w`为宽索引，后面跟两个字节，按上面所述规则拼接在一起，取值为 0~65535。`ldc2_w`则专门取`long`和`double`类型的常量。
...
* 加、减、乘、除、取余、求反指令：Tadd、Tsub、Tmul、Tdiv、Trem、Tneg；这些指令从操作数栈顶取2个元素，做运算后将结果压入栈中；
...
对于这五种情况，若两个数都是正常的数，则大于压1，等于压0，小于压-1；对于后4种，只要有一个NaN，则后缀为`g`的压1，为`l`的压-1。
