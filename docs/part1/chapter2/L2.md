...
### 6.循环展开
在进行循环的时候，可以适当地将循环变量跳转得快一些，而这些跳转的步长由接下的代码手动实现，即为**循环展开**<sup>[4]</sup>。适当的利用循环展开不仅可以降低可读性，有时候还可以提高代码运行效率。但考虑到可能存在的前向依赖，循环展开本身的实现恐怕并不是很容易。
...
在插件中混入一个特殊的类，该类以近似注入的方式让部分GUI反编译器(对! 我说的就是jd-gui!)直接崩溃。
