...
由此可见，在中间的**switch语句**的分发作用下，各分支执行相应的语句，然后再跳转回收到switch里。这样就可以把循环、判断等语句统统改写成分发-回收语句。这样生成的控制流图可以说逆向难度极大。
...
### 5.利用工具法
**你从来不是孤身一人**，通过寻求工具的帮助，加入到集体中，可以让逆向工作更加容易。

事实上，编写mod的就是利用工具的一种具体形式。参考[这篇文章](https://www.mcbbs.net/thread-1205392-1-1.html)<sup>[8]</sup>，你能近距离地感受这种方式的强大。
...
8. 由现象到本质的Minecraft源码注入艺术 https://www.mcbbs.net/thread-1205392-1-1.html